  ...
  public void Shoot(int dif)
    {
        ...
        if (randy <= auxDif) //Neutral or bad item
        {
            if(randy%2 == 0)
            {
                ...
                Instantiate(badFood, firepoint.position, Quaternion.identity);
            }
            else
            {
                Debug.Log("Spawnar item neutro");
                Instantiate(neutralFood, firepoint.position, Quaternion.identity);
            }
        }
        else
        {
            Debug.Log("Spawnar item bom");
            Instantiate(goodFood, firepoint.position, Quaternion.identity);
        }
        Debug.Log("vel = " + vel);
    }
}
